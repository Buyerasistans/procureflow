...
@pytest.mark.asyncio
async def test_ai_engine_uses_template_for_real_project_wall_layer(monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    ...
    assert result["karar_destek_sorulari"]
    assert any(
        "astar" in item["soru"].lower() and "boya" in item["soru"].lower()
        for item in result["karar_destek_sorulari"]
    )
    assert any("Astar" in item["kalem"] for item in result["recete_onerileri"])
