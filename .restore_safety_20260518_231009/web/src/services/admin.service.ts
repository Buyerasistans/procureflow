...
export interface Tenant {
  ...
  onboarding_approval_status?: string | null;
  ...
  category_requests?: Array<{ id?: number; category?: string; name?: string; applies_to?: string; status?: string; note?: string | null }>;
...
export interface CommercialRequestItem {
  id: number;
  request_type: "addon_purchase" | "package_upgrade" | "demo" | string;
...
export async function deleteTenant(id: number): Promise<{ status?: string; message?: string }> {
  const res = await http.delete<{ status?: string; message?: string }>(`/admin/tenants/${id}`);
  return res.data;
}
...
export async function updateCommercialRequestStatus(
  requestId: number,
  payload: { status: string; owner_name?: string; mark_contacted_now?: boolean },
): Promise<CommercialRequestItem> {
  const res = await http.patch<CommercialRequestItem>(`/admin/commercial-requests/${requestId}`, payload);
  return res.data;
}
...
): Promise<SubscriptionAddonAdminItem> {
  const res = await http.patch<SubscriptionAddonAdminItem>(`/admin/subscription-addons/${addonId}/lifecycle`, payload);
  return res.data;
}

export async function getCommercialRequestWebhookSettings(): Promise<CommercialRequestWebhookSettings> {
  const res = await http.get<CommercialRequestWebhookSettings>("/admin/commercial-requests/webhook-settings");
  return res.data;
}

export async function updateCommercialRequestWebhookSettings(payload: {
  webhook_url?: string | null;
  webhook_secret?: string;
  clear_webhook_secret?: boolean;
}): Promise<CommercialRequestWebhookSettings> {
  const res = await http.put<CommercialRequestWebhookSettings>("/admin/commercial-requests/webhook-settings", payload);
  return res.data;
}

export async function getCommercialRequestWebhookDeliveries(): Promise<CommercialRequestWebhookDelivery[]> {
  const res = await http.get<CommercialRequestWebhookDelivery[]>("/admin/commercial-requests/webhook-deliveries");
  return res.data;
}

export async function retryCommercialRequestWebhookDelivery(deliveryId: number): Promise<CommercialRequestWebhookDelivery> {
  const res = await http.post<CommercialRequestWebhookDelivery>(`/admin/commercial-requests/webhook-deliveries/${deliveryId}/retry`);
  return res.data;
}

export async function verifyTenantOnboardingPayment(tenantId: number): Promise<Tenant> {
  const res = await http.post<Tenant>(`/admin/tenants/${tenantId}/onboarding/verify-payment`);
  return res.data;
}

export async function approveTenantOnboardingActivation(tenantId: number): Promise<Tenant> {
  const res = await http.post<Tenant>(`/admin/tenants/${tenantId}/onboarding/approve-activation`);
  return res.data;
}

export async function rejectTenantOnboardingActivation(tenantId: number, payload?: { reason?: string } | string): Promise<Tenant> {
  const body = typeof payload === "string" ? { reason: payload } : payload || {};
  const res = await http.post<Tenant>(`/admin/tenants/${tenantId}/onboarding/reject-activation`, body);
  return res.data;
}

export async function reviewTenantCategoryRequest(
  tenantId: number,
  requestIdOrPayload: number | { category_name: string; decision: string; note?: string },
  payload?: { status: string; note?: string },
): Promise<Tenant> {
  if (typeof requestIdOrPayload === "number") {
    const res = await http.post<Tenant>(`/admin/tenants/${tenantId}/category-requests/${requestIdOrPayload}/review`, payload || {});
    return res.data;
  }
  const res = await http.post<Tenant>(`/admin/tenants/${tenantId}/category-requests/review`, requestIdOrPayload);
  return res.data;
}

export async function requestTenantOnboardingInfo(tenantId: number, payload?: { note?: string } | string): Promise<Tenant> {
  const body = typeof payload === "string" ? { note: payload } : payload || {};
  const res = await http.post<Tenant>(`/admin/tenants/${tenantId}/onboarding/request-info`, body);
  return res.data;
}

export async function getWorkspacePanelConfig(): Promise<WorkspacePanelConfig> {
  const res = await http.get<WorkspacePanelConfig>("/admin/workspace-panel-config");
  return res.data;
}

export async function updateWorkspacePanelConfig(config: WorkspacePanelConfig): Promise<WorkspacePanelConfig> {
  const res = await http.put<WorkspacePanelConfig>("/admin/workspace-panel-config", config);
  return res.data;
}
