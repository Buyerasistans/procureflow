...
export default function SupplierPersonalProfilePage() {
  ...
  const safePhone = userPhone.replace(/[^0-9+\-\s()]/g, "");
  const waPhone = safePhone.replace(/[\s\-\(\)]/g, "").replace(/^0/, "90").replace(/^\+/, "");
                          ...
                          <span className={`sppp-field__value${!userPhone ? " sppp-field__value--muted" : ""}`}>
                            {userPhone ? (
                              <span className="sppp-phone-row">
                                <a href={`tel:${safePhone}`} className="sppp-contact-link"><Phone size={12} />{userPhone}</a>
